...
*/
	...
	$("#contents .refund .selectBox ul li, #container2 .refund .selectBox ul li").on("click", function () {
		var thVar = $(this).text();
		//console.log(thVar);
		$(this).closest(".refund").find("input[type='hidden']").val(thVar);
	...
	$("#contents .refund .selectBox ul li:last, #container2 .refund .selectBox ul li:last").on("click", function () {
		$(this).closest(".refund").find("input[type='hidden']").val("dr");
	...
	/*계약 신청인 정보 동일 체크박스*/
	/*정보입력창 체크박스 추가*/
	$("[id*=container] #mng + span, [id*=container] #les + span, #cork + span, .chec_p input + span").on('click', function () {
		$(this).toggleClass("on");
	...
	$("[id*=container] #mng + span + label, [id*=container] #les + span + label, #cork + span + label, .chec_p input + span + label").on('click', function () {
